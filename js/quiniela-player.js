...
(function () {
    ...
    if (!u) {
      try {
        ...
        if (cached) {
          ...
          if (parsed && (parsed.email || parsed.uid)) u = parsed;
        }
      } catch (e) {}
    }
    return u;
  ...
  function checkQuinielaLockStatus(q) {
    ...
    if (allDone) {
      return { isLocked: true, isFinished: true, reason: 'La jornada ha finalizado con todos los partidos concluidos', earliestTime: null };
    }

    if (q.locked === true) {
      return { isLocked: true, isFinished: false, reason: 'Quiniela bloqueada por el administrador', earliestTime: null };
    }

    if (rawMatches.length === 0) {
      return { isLocked: false, isFinished: false, reason: 'Sin partidos', earliestTime: null };
    }
    ...
    if (anyStarted) {
      return { isLocked: true, isFinished: false, reason: 'La jornada ya inició con el primer partido en curso o finalizado', earliestTime: null };
    }
    ...
    if (earliestTime && Date.now() >= earliestTime) {
      return { isLocked: true, isFinished: false, reason: 'La fecha y hora del primer partido ya comenzó', earliestTime };
    }
    ...
    if (q.lockDeadline) {
      ...
      if (!isNaN(dl) && Date.now() >= dl) {
        return { isLocked: true, isFinished: false, reason: 'Se alcanzó la fecha límite para pronósticos', earliestTime: dl };
      }
    ...
    return { isLocked: false, isFinished: false, reason: 'Abierta', earliestTime };
  }
  ...
  function renderQuinielasCatalog() {
    ...
    filtered.forEach(q => {
      ...
      const isMine = !!myParticipations[q.id];
      const hasLive = matches.some(m => m.status === 'in');
      const allDone = isFinished || (matches.length > 0 && matches.every(m => m.completed === true || m.status === 'post' || (m.homeScore !== null && m.awayScore !== null && m.status !== 'in')));
      ...
      if (hasLive) {
        ...
        statusBadge = `<span class="badge" style="background:rgba(255,215,0,0.18); border:1.5px solid #ffd100; color:#ffd100; font-weight:900; box-shadow:0 0 10px rgba(255,209,0,0.2);">🏁 FINALIZADA</span>`;
      } else if (isLocked) {
      ...
      card.innerHTML = `
        ...
        <div class="q-catalog-actions" style="margin-top:14px; display:flex; gap:8px;">
          ${(() => {
            const user = getActiveUser();
            const isGoogleUser = !!(user && (user.email || user.photoURL));
            const myReg = user ? myParticipations[q.id] : null;
            const isApproved = myReg && (myReg.approved === true || myReg.status === 'approved');
            const isPending = myReg && !isApproved;

            if (!isGoogleUser) {
              return `<button class="btn btn-primary" data-login-q="${q.id}" style="flex:1; padding:11px 14px; font-size:13px; font-weight:900; border-radius:10px; background:linear-gradient(135deg, #4285F4, #1a73e8); color:#fff; display:inline-flex; align-items:center; justify-content:center; gap:8px;">
                <svg viewBox="0 0 24 24" style="width:18px; height:18px;"><path fill="#ffffff" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v4.51h6.6c-.29 1.52-1.14 2.82-2.4 3.68v3.05h3.88c2.27-2.09 3.66-5.17 3.66-9.17z"/><path fill="#ffffff" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.88-3.05c-1.08.72-2.45 1.16-4.05 1.16-3.12 0-5.77-2.1-6.72-4.93H1.25v3.15C3.26 21.36 7.33 24 12 24z"/><path fill="#ffffff" d="M5.28 14.27c-.25-.72-.38-1.49-.38-2.27s.13-1.55.38-2.27V6.58H1.25C.45 8.18 0 9.99 0 12s.45 3.82 1.25 5.42l4.03-3.15z"/><path fill="#ffffff" d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.33 0 3.26 2.64 1.25 6.58l4.03 3.15c.95-2.83 3.6-4.98 6.72-4.98z"/></svg>
    ...
    grid.querySelectorAll('[data-login-q]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
  ...
  function selectQuinielaToJoin(qId) {
    ...
    const defaultNick = (activeUser.displayName || localStorage.getItem('player_nick') || '').toUpperCase();
    ...
    container.innerHTML = `
      <div class="q-inline-join-box animate-fade">
        ...
        <div style="display:flex; align-items:center; gap:10px; background:rgba(255,255,255,0.04); padding:10px 12px; border-radius:10px; margin-bottom:12px; border:1px solid rgba(255,255,255,0.08);">
          <img src="${activeUser.photoURL || 'img/logo.jpg'}" alt="${activeUser.displayName}" style="width:36px; height:36px; border-radius:50%; object-fit:cover; border:1.5px solid #ffd100;" onerror="this.src='img/logo.jpg'"/>
          <div>
            <div style="font-weight:900; font-size:13px; color:#ffffff;">${activeUser.displayName || 'Jugador'}</div>
            <div style="font-size:11px; color:var(--text-muted);">${activeUser.email || ''}</div>
          </div>
        </div>

        <div class="q-inline-field">
          <label class="q-inline-label">👤 Apodo para la Tabla (Opcional):</label>
          <input type="text" id="inp_inline_nick_${q.id}" class="q-inline-input" value="${defaultNick}" placeholder="EJ. BETO / EL TIGRE" maxlength="25" style="text-transform:uppercase; font-weight:800;" />
        </div>

        <div class="q-inline-field">
          <label class="q-inline-label" style="color:#aaa;">🍽️ Mesero / Mesa (Opcional):</label>
          <input type="text" id="inp_inline_waiter_${q.id}" class="q-inline-input" placeholder="Ej. Mesa 4 / Carlos" maxlength="30" />
        </div>
  ...
  async function executeJoinQuiniela(qId, customNick, waiter, btnId) {
    ...
    const nick = (customNick || activeUser.displayName || 'Jugador').trim().toUpperCase();
    ...
    try {
      const q = allQuinielas.find(x => x.id === qId) || activeQuiniela;
      // In testing / default: auto-approve is true (unless explicitly set false by admin)
      const isAutoApprove = q ? (q.autoApprove !== false) : true;
      ...
      const playerData = {
        ...
        playerName: nick,
        nickname: nick,
        photoURL: activeUser.photoURL || 'img/logo.jpg',
        userPhoto: activeUser.photoURL || 'img/logo.jpg',
      ...
      if (isAutoApprove) {
        showToast(`🎉 ¡Bienvenido "${nick}" a "${q?.name || 'la quiniela'}"! Tu entrada fue aprobada.`);
      } else {
      ...
      renderQuinielasCatalog();
      if (activeQuiniela && activeQuiniela.id === qId) {
        updateQuinielaView(activeQuiniela);
      }
    } catch (err) {
      ...
      if (btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = '✅ Enviar Solicitud de Entrada'; }
    }
  }
    ...
    if (authUid) {
      try {
        ...
        if (s.exists) {
          const data = s.data();
          picks = data.picks || {};
          playerName = data.playerName || user.displayName || playerName;
  ...
  function renderLockBannerAndTimer(lockInfo) {
    ...
    if (lockInfo.isFinished) {
      if (banner) {
        banner.className = 'q-lock-banner finished';
        banner.querySelector('span').textContent = '🏆';
      }
      if (titleEl) {
        titleEl.textContent = '🏁 Jornada Finalizada — Resultados Oficiales';
        titleEl.style.color = '#ffd100';
      }
      if (subEl) {
        subEl.textContent = 'Todos los partidos han concluido. ¡Revisa al Campeón de la Quiniela y las posiciones oficiales abajo!';
      }
      if (timerContainer) timerContainer.style.display = 'none';
      if (badgeEl) {
        badgeEl.className = 'badge';
        badgeEl.style.cssText = 'background:rgba(255,215,0,0.2); border:1px solid #ffd100; color:#ffd100; font-weight:900;';
        badgeEl.textContent = '🏁 FINALIZADA';
      }
      if (btnSave) {
        btnSave.disabled = true;
        btnSave.textContent = '🏁 Jornada Finalizada';
      ...
      if (titleEl) {
        titleEl.textContent = 'Jornada Iniciada — Pronósticos Bloqueados';
  ...
  function renderPicksForm(q, isLocked) {
    ...
    if (isNotJoined) {
      ...
      if (!activeUser || !activeUser.email) {
        formEl.innerHTML = `
          <div class="card" style="border:2px solid #4285F4; background:linear-gradient(145deg, rgba(16,24,40,0.96), rgba(10,15,28,0.98)); padding:26px 18px; text-align:center; border-radius:16px; margin-bottom:14px; box-shadow:0 8px 30px rgba(0,0,0,0.5);">
            <div style="font-size:42px; margin-bottom:10px;">🏆</div>
            <h3 style="color:#ffd100; margin-bottom:6px; font-weight:900; font-size:18px;">¡Únete a "${q.name}"!</h3>
            <p style="font-size:13px; color:var(--text-muted); margin-bottom:18px; line-height:1.4; max-width:340px; margin-left:auto; margin-right:auto;">
              Para guardar tus pronósticos, verificar tu participación y competir en la tabla en vivo, inicia sesión con tu cuenta de Google.
            </p>
            <button type="button" class="btn btn-primary" onclick="if(typeof window.loginWithGoogle==='function') window.loginWithGoogle();" style="font-size:14px; font-weight:900; padding:13px 24px; border-radius:12px; display:inline-flex; align-items:center; justify-content:center; gap:10px; background:linear-gradient(135deg, #4285F4, #1a73e8); color:#fff; width:100%; max-width:320px; margin:0 auto; box-shadow:0 4px 16px rgba(66,133,244,0.4); cursor:pointer;">
              <svg viewBox="0 0 24 24" style="width:20px; height:20px;"><path fill="#ffffff" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v4.51h6.6c-.29 1.52-1.14 2.82-2.4 3.68v3.05h3.88c2.27-2.09 3.66-5.17 3.66-9.17z"/><path fill="#ffffff" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.88-3.05c-1.08.72-2.45 1.16-4.05 1.16-3.12 0-5.77-2.1-6.72-4.93H1.25v3.15C3.26 21.36 7.33 24 12 24z"/><path fill="#ffffff" d="M5.28 14.27c-.25-.72-.38-1.49-.38-2.27s.13-1.55.38-2.27V6.58H1.25C.45 8.18 0 9.99 0 12s.45 3.82 1.25 5.42l4.03-3.15z"/><path fill="#ffffff" d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.33 0 3.26 2.64 1.25 6.58l4.03 3.15c.95-2.83 3.6-4.98 6.72-4.98z"/></svg>
      ...
      formEl.innerHTML = `
        <div class="card" style="border:2px solid var(--accent-color); background:linear-gradient(145deg, rgba(30,26,16,0.96), rgba(18,16,10,0.98)); padding:24px 18px; text-align:center; border-radius:16px; margin-bottom:14px;">
          ...
          <div style="display:flex; align-items:center; justify-content:center; gap:10px; background:rgba(255,255,255,0.04); padding:10px 14px; border-radius:12px; margin:0 auto 16px auto; max-width:340px; border:1px solid rgba(255,255,255,0.08); text-align:left;">
            <img src="${activeUser.photoURL || 'img/logo.jpg'}" alt="${activeUser.displayName}" style="width:38px; height:38px; border-radius:50%; object-fit:cover; border:1.5px solid #ffd100;" onerror="this.src='img/logo.jpg'"/>
            <div>
              <div style="font-weight:900; font-size:13px; color:#ffffff;">${activeUser.displayName || 'Jugador Google'}</div>
              <div style="font-size:11px; color:var(--text-muted);">${activeUser.email || ''}</div>
            </div>
          </div>

          <div style="max-width:340px; margin:0 auto 16px auto; text-align:left;">
            <label style="font-size:12px; font-weight:900; color:#ffd100; display:block; margin-bottom:4px;">👤 Apodo para la Tabla (Opcional):</label>
            <input type="text" id="inp_detail_nick_${q.id}" class="q-inline-input" value="${defaultNick}" placeholder="EJ. BETO / EL TIGRE" maxlength="25" style="width:100%; box-sizing:border-box; margin-bottom:12px; text-transform:uppercase; font-weight:800;" />

            <label style="font-size:12px; font-weight:800; color:#aaa; display:block; margin-bottom:4px;">🍽️ Mesero / Mesa (Opcional):</label>
            <input type="text" id="inp_detail_waiter_${q.id}" class="q-inline-input" placeholder="Ej. Mesa 4 / Carlos" maxlength="30" style="width:100%; box-sizing:border-box;" />
          </div>
  ...
  async function savePlayerPicks() {
    ...
    const name = (nameInp ? nameInp.value : '').trim().toUpperCase() || existingDoc.playerName || existingDoc.userName || activeUser.displayName || 'Jugador';
    ...
    try {
      ...
      const pickData = {
        ...
        playerId: activeUser.uid,
        userUid: activeUser.uid,
        userEmail: activeUser.email || existingDoc.userEmail || '',
        photoURL: activeUser.photoURL || existingDoc.photoURL || 'img/logo.jpg',
        userPhoto: activeUser.photoURL || existingDoc.userPhoto || 'img/logo.jpg',
        userName: activeUser.displayName || name,
        playerName: name,
        nickname: name,
        waiter: existingDoc.waiter || 'Sin mesero',
        picks: newPicks,
  ...
  function renderLiveStandings(picksSnap, rawMatches) {
    ...
    // 1. Sort matches chronologically: earliest first, latest last
    const matches = sortMatchesChronologically(rawMatches || []);
    ...
    // 4. Render Winner Announcement Card
    const winnerBannerEl = document.getElementById('qWinnerBanner');
    ...
    if (winnerBannerEl) {
      if (allDone && approvedPlayers.length > 0) {
        ...
        winnerBannerEl.innerHTML = `
          <div class="q-winner-card">
            <div style="font-size:36px; margin-bottom:4px; filter: drop-shadow(0 0 12px rgba(255,209,0,0.7));">👑 🏆</div>
            <div style="font-size:12px; font-weight:900; letter-spacing:0.12em; text-transform:uppercase; color:#ffd100; margin-bottom:6px;">
              ¡TENEMOS GANADOR OFICIAL!
            </div>
            <div style="display:flex; align-items:center; justify-content:center; gap:14px; margin:12px 0;">
              <img src="${champ.photoURL || 'img/logo.jpg'}" onerror="this.src='img/logo.jpg'" style="width:58px; height:58px; border-radius:50%; border:2.5px solid #ffd100; object-fit:cover; box-shadow:0 0 18px rgba(255,209,0,0.45);"/>
              <div style="text-align:left;">
                <div style="font-size:20px; font-weight:900; color:#ffffff; text-shadow:0 0 12px rgba(255,209,0,0.35);">${champ.playerName || champ.nickname || champ.name || 'Campeón'}</div>
                <div style="font-size:14px; font-weight:800; color:#ffd100; margin-top:2px;">
                  🥇 ${champ.totalPoints} Puntos &bull; ${champ.exactHits || 0} Aciertos Exactos
                </div>
              </div>
            </div>
            <div style="font-size:12px; color:var(--text-muted); background:rgba(0,0,0,0.35); border-radius:8px; padding:7px 14px; display:inline-block; margin-top:4px;">
              🎉 ¡Felicidades al Campeón de la Quiniela! Presenta tu pantalla en barra para reclamar tu premio 🍻🍔
            </div>
          </div>
        `;
      } else {
    ...
    players.forEach((p, idx) => {
      ...
      tr.style.cssText = `border-bottom:1px solid rgba(255,255,255,0.05); ${isMe ? 'background:rgba(255,102,0,0.1);' : ''} ${!isApproved ? 'opacity:0.6;' : ''}`;

      let playerCell = `
        <td style="padding:10px 12px; position:sticky; left:0; background:${isMe ? '#1e2430' : '#121824'}; z-index:4; border-bottom:1px solid rgba(255,255,255,0.05);">
          <div style="display:flex; align-items:center; gap:8px;">
            ...
            <div style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:90px;">
              <span style="font-size:12px; font-weight:800; color:${isMe ? 'var(--accent-color)' : '#fff'};">${name} ${isMe ? '(Tú)' : ''}</span>
      ...
      let ptsCell = `
        <td style="text-align:center; padding:10px 8px; font-size:15px; font-weight:900; color:#ffd100; border-left:1px solid rgba(255,255,255,0.08); border-bottom:1px solid rgba(255,255,255,0.05); background:${isMe ? 'rgba(255,102,0,0.15)' : 'rgba(255,255,255,0.02)'};">
          ${p.totalPoints}
        </td>
      `;

      let cells = playerCell + ptsCell;
      const isJornadaStarted = checkQuinielaLockStatus(activeQuiniela).isLocked;
      ...
      matches.forEach(m => {
        ...
        if (!pick) {
          cells += `<td class="q-s-cell q-cell-neutral" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05); color:var(--text-muted); font-size:11px;">—</td>`;
          return;
        }
        ...
        if (sport === 'soccer' && !isHybrid) {
          ...
          const isHidden = !isJornadaStarted && !isMe;
          const displayStr = isHidden ? '🔒' : pickStr;

          if (!hasScore || isHidden) {
            cells += `<td class="q-s-cell q-cell-neutral" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);">${displayStr}</td>`;
            return;
          }
          ...
          if (isLive) {
            if (exact) {
              cells += `<td class="q-s-cell q-cell-live-winning" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="En Vivo: ¡Marcador Exacto! (+3 pts)">${displayStr}</td>`;
            } else if (realWin === pickWin) {
              cells += `<td class="q-s-cell q-cell-live-tied" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="En Vivo: Ganador Correcto (+1 pt)">${displayStr}</td>`;
            } else {
              cells += `<td class="q-s-cell q-cell-live-losing" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="En Vivo: Perdiendo (0 pts)">${displayStr}</td>`;
            }
          } else {
            if (exact) {
              cells += `<td class="q-s-cell q-cell-exact-hit" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="Final: ¡Exacto! (+3 pts)">🎯 ${displayStr}</td>`;
            } else if (realWin === pickWin) {
              cells += `<td class="q-s-cell q-cell-winner-hit" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="Final: Ganador (+1 pt)">✓ ${displayStr}</td>`;
            } else {
              cells += `<td class="q-s-cell q-cell-final-miss" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);" title="Final: Fallado (0 pts)">✗ ${displayStr}</td>`;
            }
          }
        } else {
          // US Sports / Pick Winner mode
          const winnerSide = pick.winner ? pick.winner : (Number(pick.homeScore) > Number(pick.awayScore) ? 'home' : 'away');

          if (!isJornadaStarted && !isMe) {
            cells += `<td class="q-s-cell q-cell-neutral" style="border-left:1px solid rgba(255,255,255,0.04); border-bottom:1px solid rgba(255,255,255,0.05);">🔒</td>`;
            return;
          }
  ...
  async function syncESPNLiveScores(quinielaId) {
    ...
    try {
      ...
      // 5. Update matches with SCORE PRESERVATION (Never wipe out valid scores with null)
